...
void set_cpu_param(cpu_param_t *param, cpu_json_t *jdata)
{
    ...
    float tmp_facotor = 1.2f*2.0f*param->grid_factor/((float)(param->grid_ydim/param->gpu_num));
    param->gpu_data_size =(int)((float)(jdata->ptc_num/param->gpu_num)*(1.0f + tmp_facotor));
    param->gpu_buff_size =(int)((float)(jdata->ptc_num/param->gpu_num)*tmp_facotor);
    //path setting
